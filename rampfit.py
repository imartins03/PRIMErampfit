...
def get_ramp_slope(frame_list,superbias, calFile, mask, slc=((4,4092), (4,4092)),degrees=4, saturation=50000):  # ((y1,y2), (x1,x2))
    ...
    bad_pix = sum_sat > 0
    y[:, bad_pix] = 0
    coefficients, cov_mat = np.polyfit(x[:-50], y[:-50,:], degrees, cov=True)
...
coeff_images = get_ramp_slope(full_file_list,supercpy,calFile,mask,degrees=6)
...
#     coeff_images = []
#     cov_matrices = []
#     coefficients, cov_mat = np.ma.polyfit(x, y, degrees,cov=True)
#
#     #ISSUE HERE WITH THE COVARIANCE
#     for coeff in coefficients:
#         coeff_image = coeff.reshape(out_img.shape)
...
#     return coeff_images, cov_matrices
#
# coeff_images,cov_matrices = get_ramp_slope(full_file_list,supercpy,calFile,mask,degrees=1)
