...
def generate_fit_cube(degree, saturation=50000, n_frames=100, num_rows=4):
    ...
    for i in range(num_rows):
        ...
        fit_coeff_path = f'F:/legfit/fit_coeff_leg_{degree}deg_row_{i}_vst.fits'
        fits.writeto(fit_coeff_path, fit_coeff, overwrite=True)  # Save coefficients
        print(f'Coefficients shape for row {i}:', fit_coeff.shape)
        ...
        fit_cube_path = f'F:/legfit/fit_cube_leg_{degree}deg_row_{i}_vst.fits'
    ...
    fit_cube_path = f'F:/legfit/fit_cube_leg_{degree}deg_noframe1_vst.fits'
