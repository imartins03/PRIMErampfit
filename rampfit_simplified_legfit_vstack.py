...
def save_coefficients(coefficients, degree, row, output_dir):
    coeffs_path = os.path.join(output_dir, f'coefficients_{n_frames}frames_degree{degree}_row{row}.fits')
...
def save_fit_cube_row(fit_cube_row, degree, row, output_dir):
    ...
    fit_cube_row_path = os.path.join(output_dir, f'fit_cube_row{row}_{n_frames}frames_degree{degree}.fits')
...
def generate_fit_cube(degree, saturation=50000, n_frames=n_frames, num_rows=4):
    ...
    output_dir = f'F:/legfit/{n_frames}_frames'
    ...
    fit_cube_path = os.path.join(output_dir, f'fit_cube_leg_{n_frames}frames_{degree}deg_final_vst.fits')
