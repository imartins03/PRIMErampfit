...
rms_slope_variance_table_path = r'F:\leftover_C1_dif_degrees_test_rampfit\rms_slope_variance_statistics_attempt.csv'
...
def compute_statistics(residuals_cube, fit_coeff, initial_frame_label):
    ...
    slope_variance = np.var(slope_vals)
    sqrt_slope_variance = np.sqrt(slope_variance)

    return rms_of_avg, avg_slope, slope_variance, sqrt_slope_variance

def save_rms_of_average_and_slope_statistics(degree, rms_of_avg, avg_slope, slope_variance, sqrt_slope_variance):
    rms_slope_df = pd.DataFrame({
        'DegreeOfFit': [degree],
        'RMSofAverage': [rms_of_avg],
        'SlopeOfFit': [avg_slope],
        'SlopeVariance': [slope_variance],
        'SqrtSlopeVariance': [sqrt_slope_variance]
    })
...
for degree in range(1, total_degrees + 1):
    ...
    rms_of_avg, avg_slope, slope_variance, sqrt_slope_variance = compute_statistics(residuals_cube, fit_coeff, initial_frame_label)  # Compute statistics

    save_rms_of_average_and_slope_statistics(degree, rms_of_avg, avg_slope, slope_variance, sqrt_slope_variance)
