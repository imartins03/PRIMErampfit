...
def compute_statistics(residuals_cube, fit_coeff, initial_frame_label):
    ...
    slope_image = fit_coeff[0]  # Extract the slope image
    ...
    rms_of_avg = np.sqrt(np.sum(np.array(rms_vals) ** 2))  # RMS of average residuals

    # Flatten the slope image and compute SEM
    ...
    # Compute the mean slope across frames for additional statistics
    avg_slope = np.mean(flattened_slope_values)  # Mean of the flattened slope values
