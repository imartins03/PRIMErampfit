...
def compute_statistics(residuals_cube, fit_coeff, initial_frame_label):
    ...
    table_save = table.to_csv(stat_table_template, mode='a', header=not pd.io.common.file_exists(rms_slope_table_path), index=True)

    total_rms_square_sum = np.sum((table_save['RMS']) ** 2)
    length_of_data = n_frames
    divisor = np.sqrt(length_of_data - 1)
    rms_of_avg = np.sqrt(total_rms_square_sum)/divisor

    return table,rms_of_avg

def save_rms_of_average_and_slope_statistics(degree, rms_of_avg, slope):
    rms_slope_df = pd.DataFrame({'Degree of Fit': [degree], 'RMS of Average': [rms_of_avg],'Slope of the Fit': [slope]})

    # Append to CSV files
    rms_slope_df.to_csv(rms_slope_table_path, mode='a', header=not pd.io.common.file_exists(rms_slope_table_path), index=True)
