...
def evaluate_poly_array(coeffs, a_array, poly_type='power'):
    output_arrays = []
    for a in a_array:  # a_array is an array of input values
        print(a)
        if poly_type == 'power':
            output_array = np.zeros(coeffs.shape[1])
            for n, coeff in enumerate(coeffs):
                output_array += coeff * (a ** n)  # Evaluate polynomial
...
for i in range(res.shape[0]):
    data = res[i]
    means.append(np.mean(data))  # Calculate mean of residuals
    rms_vals.append(np.sqrt(np.mean(data ** 2)))  # Calculate RMS of residuals
    median_vals.append(np.median(data))  # Calculate median of residuals
    std_vals.append(np.std(data))  # Calculate standard deviation of residuals
