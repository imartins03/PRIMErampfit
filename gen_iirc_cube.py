...
file_list = [file_format.format(n) for n in range(*r)] # Generate list of file paths using the range r
...
def irrc_correct_frame_file(filename, superbias, calFile, multiThread=True, externalPixelFlags=None, superbias_corrected=True):
    ...
    return irrc_correct_frame(dcpy, superbias, calFile, multiThread, externalPixelFlags, superbias_corrected)  # Correct the frame

def generate_y_cube(frame_list, superbias, calFile, slc=((4, 4092), (4, 4092))):
    y = []  #Initialize list to hold corrected frames

    for frame in frame_list:
        out_img = irrc_correct_frame_file(frame, superbias, calFile)  # Corrected frame using IRRC and calibration file
        if slc is not None:
            out_img = out_img[:, slc[0][0]:slc[0][1], slc[1][0]:slc[1][1]]  # Slice corrected frame to take out ref pix
